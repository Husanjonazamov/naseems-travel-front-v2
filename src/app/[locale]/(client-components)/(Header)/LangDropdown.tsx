...
const LangDropdown: FC<LangDropdownProps> = ({
  ...
  const router: any = useRouter(); // vaqtinchalik any qilib qo‘ydik
  const pathname = usePathname();
  const t = useTranslations("travel");

  const currentLocale = pathname?.split("/")[1] || "uz";
  const [selectedLang, setSelectedLang] = useState(
    headerLanguage.find((l) => l.id === currentLocale) || headerLanguage[0]
  );

  const handleLangChange = (item: { id: string; name: string }, close: () => void) => {
    setSelectedLang(item);
    close();

    const basePath = pathname ? pathname.replace(/^\/[a-z]{2}(\/|$)/, "/") : "/";
    const newPath = `/${item.id}${basePath}`;

    router.push(newPath); // TypeScript endi xato bermasligi kerak
  };
  ...
  const renderLang = (close: () => void) => (
    <div className="grid gap-2">
      {headerLanguage.map((item) => (
        <button
          key={item.id}
          onClick={() => handleLangChange(item, close)}
